...
fun readInputText(name: String) = File(".", "$name.txt").readText()

fun readInputLines(name: String) = File(".", "$name.txt").readLines()

fun main() {

  fun deserialize(s: String): Any {
    if ("[" !in s) return s.toInt()
    var str = ""
    var c = 0
    ...
    for (ch in s.drop(1).dropLast(1)) {
      if (ch == '[') c++
      if (ch == ']') c--
      if (c != 0 || ch != ',') str += ch else arr += deserialize(str).also { str = "" }
    }
    if (str.isNotEmpty()) arr += deserialize(str)
    return arr
  }
  ...
  val testInput = readInputLines("example")
  println(part1(testInput))
  println(part2(testInput))

  val input = readInputLines("input")
  println(part1(input))
  println(part2(input))
}
